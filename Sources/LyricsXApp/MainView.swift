...
struct MainView: View {
    ...
    private var header: some View {
        HStack(spacing: 10) {
            Image(systemName: "quote.bubble.fill").font(.system(size: 19)).foregroundStyle(Color.primary.opacity(0.85))
    ...
    private var footer: some View {
        HStack(spacing: 7) {
            ...
            if model.session.track != nil {
                ...
                Text(model.session.isPlaying ? "实时同步" : "已暂停").font(.system(size: 10)).foregroundStyle(Color.primary.opacity(0.4))
            }
            Spacer()
            if model.session.isSearching {
                ProgressView().controlSize(.mini)
                Text(model.session.document == nil ? "正在加载歌词…" : "正在加载更多版本…")
                    .font(.system(size: 10)).lineLimit(1).foregroundStyle(Color.primary.opacity(0.6))
                ...
                Text(error).font(.system(size: 10)).lineLimit(1).foregroundStyle(Color.primary.opacity(0.6)).frame(maxWidth: 290)
            } else if let doc = model.session.document {
                Text(doc.source).font(.system(size: 10)).foregroundStyle(Color.primary.opacity(0.4))
                if doc.hasWordTiming { Text("逐字").font(.system(size: 9, weight: .medium)).padding(.horizontal, 6).padding(.vertical, 3).background(Color.primary.opacity(0.07), in: .capsule) }
            }
            if let doc = model.session.document, doc.isSynced {
                Rectangle().fill(Color.primary.opacity(0.1)).frame(width: 1, height: 14).padding(.horizontal, 12)
        ...
        }.font(.system(size: 10)).buttonStyle(.plain).foregroundStyle(Color.primary.opacity(0.55))
            // Loading, instrumental and synced tracks occupy the same footer.
            // Otherwise removing the offset controls briefly adds 18pt to the
            // player, moving artwork and transport twice on every track change.
            .frame(height: 32)
